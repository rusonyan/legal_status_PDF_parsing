...
class DifferentiationModel:
    def differentiation(self, pgd, bk_path):
        state = ''
        check_tem = [
            "专利权的转移",
            "专利权全部无效",
        ...
        for c in check_tem:
            if check_string(c, pgd.title):
                state = c
        self.differentiation_handle(pgd, state, bk_path)

    def differentiation_handle(self, pgd, state, bk_path):
        title = state
        print(title + "表正在处理---<->\n")
        if check_string("专利权的转移$", title):  # 1
            PatentRightChangeIntermediary(pgd.text_block, bk_path)
        elif check_string("专利权全部无效$", title):  # 2
            PatentInvalidationIntermediary(pgd.text_block, bk_path)
        elif check_string("未缴年费专利权终止$", title):  # 3
            TerminationUnpaidAnnualFeeIntermediary(pgd.text_block, bk_path)
        elif check_string("专利权有效期届满$", title):  # 4
            ExpiryOfThePatentRightIntermediary(pgd.text_block, bk_path)
        elif check_string("专利权的主动放弃$", title):  # 5
            PatentAbandonmentIntermediary(pgd.text_block, bk_path)
        elif check_string("著录事项变更$", title):  # 6
            print("无需关注的警告：" + title + " 表暂无须解析！")
        elif check_string("专利权人的姓名或者名称、地址的变更$", title):  # 7
            PatentOwnerChangesIntermediary(pgd.text_block, bk_path)
            ...
            PatentPreservationIntermediary(pgd.text_block, bk_path)
