...
class PatentInvalidationPart:
    ...
    def Insert(self, db):
        db.cursor.execute("insert into ip (id, patent_id, decision_num, decision_date)"
                          " values (%s,%s,%s,%s);",
                          (0, db.split_patent(self.Patent_number), self.Invalidation_decision_number,
                           self.Invalidation_decision_date))

        db.cursor.execute(
            'insert into patent_change_log (id, code, pub_date, patent_num, raw_data, change_id, source)'
            'values (%s,%s,%s,%s,%s,%s,%s);',
            (0, 'IP01', self.Invalidation_decision_date, db.split_patent(self.Patent_number),
             '专利权部分无效\n无效宣告决定日:' +
             self.Invalidation_decision_date +
